/**
 * @file VmcMessages.h
 * @brief Wire format of the VMC's health-monitor reports.
 *
 * Copied verbatim from dpdk_vmc/include/health_monitor/vmc_message_types.h so
 * that this application stays self-contained. It is a description of what the
 * VMC puts on the wire, not code with a life of its own - if the two ever
 * disagree, that file is the original and this one is wrong.
 *
 * Every struct is packed and every multi-byte field is big-endian on the wire;
 * VmcHealth.c does the swapping. The static assertions at the end hold the
 * sizes the comments claim, so a compiler that lays one out differently fails
 * the build rather than producing quiet nonsense.
 */

#ifndef VMC_MESSAGES_H
#define VMC_MESSAGES_H
...
_Static_assert(sizeof(vmp_cmsw_header_t)            ==   11, "VMC header is 11 bytes");
_Static_assert(sizeof(vmc_pbit_data_t)              ==  454, "PBIT report is 454 bytes");
_Static_assert(sizeof(bm_engineering_cbit_report_t) ==  397, "BM engineering is 397 bytes");
_Static_assert(sizeof(bm_flag_cbit_report_t)        ==  105, "BM flag is 105 bytes");
_Static_assert(sizeof(dtn_es_cbit_report_t)         ==  352, "DTN ES CBIT is 352 bytes");
