/*
 * The VMC health-monitor report printers.
 *
 * Lines 924-2126 of dpdk_vmc/src/health_monitor/health_monitor.c, verbatim.
 * Nothing here is adapted: the point is that a report printed by this
 * application and the same report printed by dpdk_vmc are the same text, so
 * the two can be compared line for line. If the two ever disagree, that file
 * is the original and this one is wrong.
 *
 * The functions take an already-parsed report. Receiving, sorting and
 * byte-swapping happen in VmcHealth.c, which parses exactly as that file does.
 *
 * Three deliberate departures, none of which touch a printed character:
 *
 *   - hm_check_bm_engineering_temps, hm_check_dtn_es_temps and
 *     hm_check_dtn_sw_temps lose their `static`, because the dashboard lives in
 *     another file here and calls them after printing, as it does there;
 *   - hm_temperature_failed() is added at the end, because the dashboard reads
 *     that flag directly there and cannot here;
 *   - the temperature limits move to VmcPrint.h, which is where the original
 *     keeps them (health_monitor.h).
 */
...
void hm_check_bm_engineering_temps(const bm_engineering_cbit_report_t *d,
                                          const char *device)
{
...
/* The one addition to this file: the dashboard has to ask whether a limit was
 * broken, and in dpdk_vmc that flag is a file-scope variable it reads directly. */
bool hm_temperature_failed(void)
{
    return g_hm_temp_failed;
}
