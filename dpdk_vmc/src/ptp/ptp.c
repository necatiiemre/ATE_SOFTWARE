...
/* Field order: role, dpdk_port, ne, sync_vlan, sync_vl_id, req_vlan, req_vl_id,
 * resp_vlan, resp_vl_id, j_label. VLANs are taken from the normal data-plane
 * flow (VMC_PORT_MAP_INIT): for M2 legs sync/resp use the J's tx_vlan and req
 * uses its rx_vlan. */
const struct ptp_flow ptp_flows[PTP_NUM_FLOWS] = {
    ...
    /* Mechanism 2 — Slave role. One Sync per NE side arrives on the group
     * "leader" leg only (FLCS=J7-4, VS=J5-4 — the single leg with a non-zero
     * sync_vlan). That Sync fans out a Delay_Req on every leg sharing the same
     * sync_vl_id, including legs on other ports (FLCS: J6-1/J6-2 on port 0;
     * VS: J6-3/J6-4 on port 0). Non-leader legs carry sync_vlan=0 (they never
     * receive a Sync of their own). */

    /* FLCS = NE A, group sync_vl_id 1600 (Sync on J7-4 / port 1 / VLAN 240) */
    { PTP_ROLE_M2_SLAVE,  1, PTP_NE_A, 240, 1600,  112, 1601,  240, 1607, "J7-4" },
    { PTP_ROLE_M2_SLAVE,  1, PTP_NE_A,   0, 1600,  111, 1602,  239, 1608, "J7-3" },
    { PTP_ROLE_M2_SLAVE,  1, PTP_NE_A,   0, 1600,  110, 1603,  238, 1609, "J7-2" },
    { PTP_ROLE_M2_SLAVE,  1, PTP_NE_A,   0, 1600,  109, 1604,  237, 1610, "J7-1" },
    { PTP_ROLE_M2_SLAVE,  0, PTP_NE_A,   0, 1600,  106, 1605,  236, 1611, "J6-2" },
    { PTP_ROLE_M2_SLAVE,  0, PTP_NE_A,   0, 1600,  105, 1606,  233, 1612, "J6-1" },

    /* VS = NE B, group sync_vl_id 1620 (Sync on J5-4 / port 3 / VLAN 232) */
    { PTP_ROLE_M2_SLAVE,  3, PTP_NE_B, 232, 1620,  104, 1621,  232, 1627, "J5-4" },
    { PTP_ROLE_M2_SLAVE,  3, PTP_NE_B,   0, 1620,  103, 1622,  231, 1628, "J5-3" },
    { PTP_ROLE_M2_SLAVE,  3, PTP_NE_B,   0, 1620,  102, 1623,  230, 1629, "J5-2" },
    { PTP_ROLE_M2_SLAVE,  3, PTP_NE_B,   0, 1620,  101, 1624,  229, 1630, "J5-1" },
    { PTP_ROLE_M2_SLAVE,  0, PTP_NE_B,   0, 1620,  108, 1625,  234, 1631, "J6-4" },
    { PTP_ROLE_M2_SLAVE,  0, PTP_NE_B,   0, 1620,  107, 1626,  235, 1632, "J6-3" },
};
...
/* Per-frame RX/TX tracing (the [PTP-rx]/[PTP-tx] lines). Off by default now
 * that the engine is validated; flip to 1 to re-enable verbose diagnosis. */
#define PTP_TRACE 0
...
/* Per-frame tracing: logs every 0x88F7 frame received on the PTP queue
 * (msg/vlan/vl-id/NE and the matched leg, or UNMATCHED) and every Delay_Req we
 * emit. Always on — at 1 Hz Sync the volume is a handful of lines/sec/port. */
...
/* Match incoming packet to a flow. The key differs per message type:
 *  - Sync: the proprietary VL-ID is shared across a group (1600/1620), so the
 *    per-J 802.1Q VLAN is the discriminator and only the group leader carries a
 *    non-zero sync_vlan.
 *  - Delay_Req/Delay_Resp: matched on the unique VL-IDX in the dst MAC, NOT the
 *    VLAN. On cross-wired legs (J6-2<->J6-4) the fixture swaps the 802.1Q VLAN
 *    tag between the pair, but the VL-IDX stays correct, so VL-ID matching
 *    routes each response to its own leg.
 * Returns flow index in [0, PTP_NUM_FLOWS) or -1. */
static int lookup_flow(uint16_t port_id, uint16_t vlan, uint16_t vl_id,
                       uint8_t msg_type)
{
    for (int i = 0; i < PTP_NUM_FLOWS; i++) {
        ...
        switch (msg_type) {
        case PTP_MSG_SYNC:
            /* Only M2 receives Sync, and only the group's sync source carries
             * a non-zero sync_vlan (one leg per NE side: J7-4 / J5-4). */
            if (f->role == PTP_ROLE_M2_SLAVE &&
                f->sync_vlan != 0 && f->sync_vlan == vlan) return i;
            break;
        case PTP_MSG_DELAY_REQ:
            /* Only M1 receives Delay_Req; match on the unique req VL-ID. */
            if (f->role == PTP_ROLE_M1_MASTER && f->req_vl_id == vl_id) return i;
            break;
        case PTP_MSG_DELAY_RESP:
            /* Only M2 receives Delay_Resp; match on the unique resp VL-ID so
             * cross-wired VLAN swaps don't cross legs. */
...
/* One Sync per NE side arrives on the group leader leg (J7-4 / J5-4). It fans
 * out a fresh Delay_Req on EVERY M2 leg that shares the same sync_vl_id — the
 * group leg set spans two ports for FLCS (J7-* on port 1, J6-1/J6-2 on port 0)
 * and for VS (J5-* on port 3, J6-3/J6-4 on port 0). T1/T2 are shared by the
 * group; each leg keeps its own T3/T4 from its Delay_Req/Delay_Resp leg. */
static void handle_sync_rx(int leader_idx,
                           const uint8_t *body,
                           uint64_t t2_ns,
                           uint64_t mono_now)
{
    uint64_t t1_ns = ptp_read_time(body + PTP_BODY_TIME1_OFF);
    uint16_t group_sync_vl_id = ptp_flows[leader_idx].sync_vl_id;

    for (int i = 0; i < PTP_NUM_FLOWS; i++) {
        const struct ptp_flow *f = &ptp_flows[i];
        if (f->role != PTP_ROLE_M2_SLAVE) continue;
        if (f->sync_vl_id != group_sync_vl_id) continue;

        struct ptp_state *st = &g_state[i];
        st->last_t1_ns           = t1_ns;
        st->last_t2_ns           = t2_ns;
        st->sync_rx++;
        ...
        if (!g_dbg_first_sync_done[i]) {
            g_dbg_first_sync_done[i] = true;
            printf("[PTP-dbg] first Sync: flow=%d port=%u NE=%c vl_id=%u T1=%lu T2=%lu\n",
                   i, (unsigned)f->dpdk_port,
                   (f->ne == PTP_NE_A) ? 'A' : 'B',
                   f->sync_vl_id,
                   (unsigned long)t1_ns,
                   (unsigned long)t2_ns);
        ...
        /* If a previous Delay_Req on this leg is still outstanding when a new
         * Sync arrives, the master never answered it — count a timeout before
         * we overwrite the timer. */
        ...
        int tx_rc = ptp_tx_frame(f->dpdk_port, buf, sizeof(buf));
        if (tx_rc == 0) {
            st->last_t3_ns           = t3;
            st->tx_seq++;
            st->req_tx++;
...
#if PTP_TRACE
        {
            printf("[PTP-tx] DelayReq leg=%-4s port=%u req_vlan=%u dst_vl=%u "
                   "t3=%lu %s\n",
                   f->j_label, (unsigned)f->dpdk_port, (unsigned)f->req_vlan,
                   (unsigned)f->req_vl_id, (unsigned long)t3,
                   (tx_rc == 0) ? "ok" : "TX-FAIL");
        }
#endif
    }
...
static void rx_one_port(uint16_t port_id)
{
    ...
    for (uint16_t i = 0; i < n; i++) {
        ...
        /* Dest MAC bytes 4..5 = VL-IDX (big-endian); 802.1Q VLAN is the
         * per-J match key (see lookup_flow). */
        ...
        int idx = lookup_flow(port_id, vlan, vl_id, msg_type);

#if PTP_TRACE
        {
            const char *mname = (msg_type == PTP_MSG_SYNC)      ? "Sync"
                              : (msg_type == PTP_MSG_DELAY_REQ) ? "Req"
                              : (msg_type == PTP_MSG_DELAY_RESP)? "Resp" : "?";
            printf("[PTP-rx] port=%u %-4s(0x%02x) vlan=%u vl_id=%u src_ne=0x%02x "
                   "len=%u -> %s\n",
                   (unsigned)port_id, mname, msg_type, (unsigned)vlan,
                   (unsigned)vl_id, src_ne, (unsigned)m->data_len,
                   (idx >= 0) ? ptp_flows[idx].j_label : "UNMATCHED");
        }
#endif
        ...
        /* VLAN already matched in lookup_flow (it is the per-J key), so no
         * separate bad_vlan check is needed here. */

        /* For Sync, validate the NE byte. Other directions originate from the
         * Unit/VMC and their src MAC convention differs. */
...
static void *ptp_thread_fn(void *arg)
{
    ...
    while (!g_ptp_stop) {
        ...
        /* Every 5 s, dump NIC-level stats for any PTP port that has seen no
         * PTP RX activity at all. Lets us tell "switch isn't sending us
         * anything" from "frames arrive but get misrouted". A port counts as
         * active on any RX: Sync (M2), Delay_Req (M1) or Delay_Resp (M2), so
         * M1-only ports (e.g. port 2) are not falsely flagged silent. */
            ...
            for (uint16_t i = 0; i < g_ptp_port_count; i++) {
                ...
                for (int k = 0; k < PTP_NUM_FLOWS; k++)
                    if (ptp_flows[k].dpdk_port == port &&
                        (g_state[k].sync_rx > 0 || g_state[k].req_rx > 0 ||
                         g_state[k].resp_rx > 0)) { any_rx = true; break; }
                if (any_rx) continue;
...
/* One-time dump of the full flow table (VLAN + VL-ID per direction) so the
 * configured addressing can be eyeballed against the wiring spec. M2 legs that
 * are not their group's Sync source carry Sync VLAN '-' (they never receive a
 * Sync of their own; the leader's Sync fans out to them). */
static void ptp_print_config(void)
{
    printf("\n[PTP] Configured flows (VLAN / VL-ID per direction):\n");
    printf("┌────┬──────┬────┬────┬──┬───────────────┬───────────────┬───────────────┐\n");
    printf("│ #  │ Lbl  │ Pt │Role│NE│ Sync VLAN/VL  │ Req  VLAN/VL  │ Resp VLAN/VL  │\n");
    printf("├────┼──────┼────┼────┼──┼───────────────┼───────────────┼───────────────┤\n");
    for (int i = 0; i < PTP_NUM_FLOWS; i++) {
        const struct ptp_flow *f = &ptp_flows[i];
        char syncv[8];
        if (f->sync_vlan == 0) snprintf(syncv, sizeof(syncv), "%4s", "-");
        else                   snprintf(syncv, sizeof(syncv), "%4u", f->sync_vlan);
        printf("│ %2d │ %-4s │ %2u │ M%c │%c │ %4s / %5u  │ %4u / %5u  │ %4u / %5u  │\n",
               i, f->j_label, (unsigned)f->dpdk_port,
               (f->role == PTP_ROLE_M1_MASTER) ? '1' : '2',
               (f->ne == PTP_NE_A) ? 'A' : 'B',
               syncv, (unsigned)f->sync_vl_id,
               (unsigned)f->req_vlan,  (unsigned)f->req_vl_id,
               (unsigned)f->resp_vlan, (unsigned)f->resp_vl_id);
    }
    printf("└────┴──────┴────┴────┴──┴───────────────┴───────────────┴───────────────┘\n");
}
...
void ptp_print_dashboard(void)
{
     ...
     * Disabled: çıktı bastırıldı (basılmasın). */
#if 0
