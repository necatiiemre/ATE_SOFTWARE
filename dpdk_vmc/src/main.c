...
int main(int argc, char const *argv[])
{
    ...
    // Preload PBIT report slots from the pre-flight test_starter snapshot so
    // the health dashboard shows the (one-shot) power-on BIT data. Missing
    // snapshots are ignored.
    hm_preload_pbit_snapshots();

    // Health monitor sıcaklık limit kontrolünü global kapatma bayrağına bağla:
    // aralık dışı bir sıcaklık görülürse dashboard force_quit'i set eder ve
    // ana döngü güvenle sonlanır.
    hm_set_abort_flag(&force_quit);
