...
public:
    ...
    /**
     * @brief Execute command on remote server, streaming its combined
     *        stdout/stderr to the local console line-by-line as it arrives
     *        (live progress) while also capturing it into *output for parsing.
     * @param command Command to execute
     * @param output Captured combined output (optional)
     * @param use_sudo Run with sudo (optional)
     * @param timeout_ms Caps total run time
     * @return true on success (exit code 0 and no timeout)
     */
    bool executeStreaming(const std::string& command, std::string* output = nullptr,
                          bool use_sudo = false, int timeout_ms = 120000);
