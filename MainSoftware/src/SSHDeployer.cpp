...
bool SSHDeployer::executeStreaming(const std::string& command, std::string* output,
                                   bool use_sudo, int timeout_ms) {
    std::string actual_command = command;
    if (use_sudo) {
        actual_command = "echo '" + m_password + "' | sudo -S " + command;
    }

    // Fold stderr into stdout so the operator sees everything, then pipe the
    // whole SSH session through popen and read it as it arrives.
    std::string ssh_cmd = buildSSHCommand(actual_command + " 2>&1");

    FILE* fp = popen(ssh_cmd.c_str(), "r");
    if (!fp) {
        std::cerr << getLogPrefix() << " executeStreaming: popen failed" << std::endl;
        return false;
    }
    int fd = fileno(fp);
    ...
    while (true) {
        ...
        if (pr == 0) { timed_out = true; break; }  // no output for the whole budget
        ...
        if (n == 0) break;  // EOF — remote command finished

        std::cout.write(buf, n);
        std::cout.flush();
        if (output) acc.append(buf, (size_t)n);
    }
    ...
    if (timed_out) {
        std::cerr << getLogPrefix() << " executeStreaming: timed out after "
