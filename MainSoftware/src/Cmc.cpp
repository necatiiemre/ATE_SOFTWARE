...
#include "DebugConfig.h"   // MAIN_DEBUG_MODE, MAIN_AUTO_CONTINUE_WAIT_S
#include "Utils.h"
#include <iostream>
#include <unistd.h>
#include <iomanip>
...
void Cmc::fetchDpdkAndMmmsLogs()
{
    // Run at most once (normal completion path OR signal-triggered shutdown).
    bool expected = false;
    if (!m_logs_fetched.compare_exchange_strong(expected, true))
    {
        DEBUG_LOG("CMC: Logs already fetched, skipping.");
        return;
    }
    ...
    std::string local_mmms_dir = g_ReportManager.getTestLogDir() + "/mmms_logs";
    ...
    if (g_ssh_deployer_server.fetchDirectory("/tmp/mmms_logs", local_mmms_dir))
    {
        ...
        if (!g_ssh_deployer_server.execute("sudo rm -rf /tmp/mmms_logs", nullptr, false, true))
        {
...
bool Cmc::configureSequence()
{
    ...
    {
        const int test_starter_timeout_s = 1200;
        ...
        std::string cmd = "cd " + remote_dir + "/test_starter_cmc && "
...
#if MAIN_DEBUG_MODE
            // Otomasyon: operatör beklenmez. Belirtilen süre kadar beklenip
            // otomatik "yes" verilir; trigger paketi olmadan teste devam edilir.
            std::cout << "[DEBUG] Teste devam sorusu otomatik: "
                      << MAIN_AUTO_CONTINUE_WAIT_S
                      << " saniye beklenip 'yes' verilecek." << std::endl;
            std::cout.flush();
            sleep(MAIN_AUTO_CONTINUE_WAIT_S);
            std::cout << "[DEBUG] Otomatik cevap: YES. Trigger paketi olmadan devam ediliyor."
                      << std::endl;
#else
    ...
    std::string flicker_log    = g_ReportManager.getTestLogDir() + "/flicker_detection.log";
    std::string flicker_output = g_ReportManager.getTestLogDir() + "/flicker_output";
    ...
    // From here on, DPDK is producing /tmp/dpdk_app.log on the server. If the
    // software is killed (e.g. bash watchdog SIGTERM every 2h) before the normal
    // completion path fetches the logs, SafeShutdown will invoke this hook after
    // stopping DPDK so the logs are still saved. Idempotent with the normal path.
    shutdown.setLogFetchCallback([this]() { fetchDpdkAndMmmsLogs(); });
    ...
    // Fetch DPDK CMC + MMMS logs to local PC (idempotent; may also run from the
    // signal-triggered SafeShutdown log-fetch callback registered below).
    fetchDpdkAndMmmsLogs();
