...
void SafeShutdown::setLogFetchCallback(std::function<void()> cb) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_log_fetch_cb = std::move(cb);
}
...
void SafeShutdown::executeShutdown() {
    ...
    // PRIORITY 5: Save device logs (DPDK / MMMS etc.) if a hook was registered.
    // Runs AFTER DPDK/remote apps are stopped so the log files are flushed.
    // On a bash-watchdog SIGTERM this is the only place logs get saved.
    if (m_log_fetch_cb) {
        ErrorPrinter::info("SYSTEM", "Saving device logs before shutdown...");
        try {
            m_log_fetch_cb();
            ErrorPrinter::info("SYSTEM", "Device logs saved.");
        } catch (...) {
            ErrorPrinter::error("SYSTEM", "Device logs could not be saved!");
        }
    }
