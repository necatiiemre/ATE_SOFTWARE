...
#include "Utils.h"
#include "DebugConfig.h"   // MAIN_DEBUG_MODE, DEBUG_UNIT
#include <algorithm>
#include <iostream>
...
{
#if MAIN_DEBUG_MODE
    // Debug/otomasyon modu: cihaz seçimi hardcoded (DebugConfig.h → DEBUG_UNIT).
    Unit dbg_unit = DEBUG_UNIT;
    std::cout << "[DEBUG] Cihaz secimi: " << enumToString(dbg_unit) << std::endl;
    return dbg_unit;
#else
    int choice = 0;
    Unit unit;
