...
A command-line tool to parse CMSW log files and generate output reports.

## Build Instructions

The project is built using `gcc`. Ensure you have a MinGW/GCC environment installed.

```bash
gcc main.c -I include -lws2_32 -o cmswlogparser.exe
```
Note: The -lws2_32 flag links the Windows Sockets library, which is required for the program's networking/socket components.
...
## Features
    ...
    Architecture Aware: Handles the byte-swapping required to translate PowerPC log data to Windows-compatible formats.
    Lightweight & Fast: Written in pure C for minimal overhead and maximum parsing speed.
    CSV Output: Converts complex binary structures into standard Comma-Separated Values, making them instantly compatible with Excel, Python (Pandas), or MATLAB.
    Minimal Dependencies: Requires only a standard C compiler and the Windows Socket library (ws2_32).

## Prerequisites

To compile and run this project, you will need:

    Compiler: gcc (MinGW-w64 recommended for Windows).
    Environment: Windows Command Prompt, PowerShell, or a Bash terminal (Git Bash/WSL).

## Workflow Example

    Input: clcmsw_detections_31_10_x_file_0.log (Raw Binary, Big-Endian).
    Process: cmswlogparser reads the binary chunks, swaps the bytes for integers/floats, and structures the data.
    Output: A generated .csv file containing the parsed detection data, ready for analysis.
...
| :--- | :--- |
| **Language** | C |
| **Source Architecture** | PowerPC (Big-Endian) |
| **Target Architecture** | x86/x64 (Little-Endian) |
| **Output Format** | CSV (Comma Separated Values) |
| **Compiler Flags** | `-lws2_32` |