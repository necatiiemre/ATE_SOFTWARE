...
from .bitfield import TYPE_BITS, process_struct_fields, BitfieldGroup, RegularField
...
def _traverse_struct(
    ...
    for elem in elements:
        if isinstance(elem, BitfieldGroup):
            if elem.unit_bits > 8:
                raw_name = f"_raw_{elem.fields[0].name}"
                base_access = f"{access_prefix}{separator}{raw_name}"
                conv_func = ENDIAN_CONVERSIONS.get(elem.dominant_type)
                if conv_func:
                    steps.append(
                        ConversionAction(
                            access_path=base_access,
                            type_name=elem.dominant_type,
        ...
        fld = elem.field
        # Full-width bitfields (e.g. uint64_t x : 64) are handled as RegularField
