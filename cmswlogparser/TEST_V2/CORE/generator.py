    ...
    for elem in elements:
        if isinstance(elem, RegularField):
            ...
            if elem.unit_bits > 8:
                ...
                lines.append("    union {")
                lines.append(f"        {elem.dominant_type} {raw_name};")
                lines.append("        struct {")
                indent = "            "
            else:
                indent = "    "

            lines.append(f"{indent[:-4]}#if defined(SYSTEM_LITTLE_ENDIAN)")
            for fld in reversed(elem.fields):
                lines.append(
                    f"{indent}{elem.dominant_type} {fld.name} : {fld.bit_width};"
                )
            lines.append(f"{indent[:-4]}#elif defined(SYSTEM_BIG_ENDIAN)")
            for fld in elem.fields:
                lines.append(
                    f"{indent}{elem.dominant_type} {fld.name} : {fld.bit_width};"
                )
            lines.append(f"{indent[:-4]}#endif")
