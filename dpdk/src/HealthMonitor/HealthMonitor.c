...
static void health_print_fpga_table(FILE *out, const char *fpga_name, const struct health_fpga_data *fpga)
{
    ...
    // Port Status Table (all per-port counters from FPGA)
    // Column widths must stay in sync with the separator line below.
    static const int port_col_widths[] = {
        4,  /* Port      */
        5,  /* BitSt     */
        6,  /* Speed     */
        9,  /* TxCnt     */
    ...
    const int port_col_count = (int)(sizeof(port_col_widths) / sizeof(port_col_widths[0]));

    printf("[HEALTH] ---- %s FPGA Port Status (pkts=%d, ports=%d) ----\n",
           fpga_name, fpga->packets_received, fpga->port_count_received);

    // Header row
    printf("[HEALTH] %4s | %5s | %6s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s | %9s |\n",
           "Port", "BitSt", "Speed", "TxCnt", "RxCnt", "CRCErr", "AliErr",
           "LenEx64", "LenEx1518", "MinVLErr", "MaxVLErr", "InPErr", "PolDrop",
           "BECnt", "VLSrcErr", "MaxDlyErr", "QOvf", "VLDrop", "UndefMAC",
           "HP_Ovf", "LP_Ovf", "BE_Ovf", "MaxDlyP");

    // Separator row (aligned to the column widths / " | " separators above)
    printf("[HEALTH] ");
    for (int c = 0; c < port_col_count; c++) {
        for (int d = 0; d < port_col_widths[c]; d++) {
            putchar('-');
        }
        printf(c < port_col_count - 1 ? "-+-" : "-+");
    }
    putchar('\n');
    ...
    for (int i = 0; i < HEALTH_MAX_PORTS; i++) {
        ...
        if (p->valid) {
            char bit_buf[8];
            snprintf(bit_buf, sizeof(bit_buf), "0x%02X", p->bit_status);
            printf("[HEALTH] %4d | %5s | %6s | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu | %9lu |\n",
                   p->port_number,
                   bit_buf,
            ...
            if (state->post_warmup_cycle_count >= 10) {
                ...
                if (cycle.assistant.device_info_valid && cycle.manager.device_info_valid) {
                    ...
                    if (ast->sw_ip_major != mgr->sw_ip_major ||
                        ...
                        printf("[HEALTH] !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                        printf("\n");
                        printf("Firmware version mismatch\n");
                        // *g_stop_flag = true;
                    } else {
