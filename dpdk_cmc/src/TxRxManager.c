...
#include "health_monitor.h" // hm_is_health_monitor_vl_id, hm_handle_packet
#include <rte_lcore.h>
...
int rx_worker(void *arg)
{
    ...
    uint64_t local_xor_fail = 0;                     // XOR-zone fail counter
    ...
    while (!(*params->stop_flag))
    {
        for (int iter = 0; iter < INNER_LOOPS; iter++)
        {
            ...
            for (uint16_t i = 0; i < nb_rx; i++)
            {
                ...
                if (vl_id <= MAX_VL_ID)
                {
                    ...
                    else
                    {
                        ...
                        if (seq > expected)
                        {
                            ...
                            printf("[P%u Q%u CMC%u] LOSS: VL=%u seq=%" PRIu64
                                   " expected=%" PRIu64 " gap=%" PRIu64 "\n",
                                   params->port_id, params->queue_id,
                                   (unsigned)worker_cmc_port,
                                   vl_id, seq, expected, gap);
                            local_lost += gap;
                            pkt_gap_loss = gap;
#if TOKEN_BUCKET_TX_ENABLED
                ...
                if (payload_mode == CMC_PAYLOAD_SPLITMIX_CRC) {
                    ...
                    if (likely(crc_ok && sm_ok && xor_ok && prbs_ok)) {
                        ...
                        if (!xor_ok) local_xor_fail++;
                        if (unlikely(!first_bad)) {
                            printf("✗ BAD: Port %u Q%u VL-ID %u Seq %lu (SM=%s CRC=%s XOR=%s PRBS=%s)\n",
                                   params->port_id, params->queue_id, vl_id, seq,
                                   sm_ok ? "OK" : "FAIL", crc_ok ? "OK" : "FAIL",
                                   xor_ok ? "OK" : "FAIL", prbs_ok ? "OK" : "FAIL");
                            first_bad = true;
                        }
                        ...
                        // [76] XOR-zone byte
                        if (!xor_ok) {
                            berr += __builtin_popcount((unsigned)(recv_xor ^ expected_xor));
                        }
                        // [77+] PRBS zone
                        if (!prbs_ok) {
            ...
            if (unlikely(local_rx >= FLUSH))
            {
                ...
                local_sm_fail = local_crc_fail = local_xor_fail = 0;
            }
        }
    }
...
int start_txrx_workers(struct ports_config *ports_config, volatile bool *stop_flag)
{
    ...
    printf("\nPacket Format:\n");
    printf("  SRC MAC: 02:00:00:00:00:XX (last byte 0x20=Net A / 0x40=Net B)\n");
