#include "AteMode.h"
#include "Config.h"     // CMC_AUTO_MODE
#include <stdio.h>
#include <unistd.h>     // sleep

static bool g_ate_mode = false;

// Otomasyon modunda ATE seçim ekranı için bekleme (saniye). 5 dakika. (saniye olarak degisti)
#define ATE_AUTO_WAIT_S (5 * 1)
...
#if CMC_AUTO_MODE
    // Otomasyon: kullanıcıya sorma. 5 dakika bekle, ardından ATE mode için
    // otomatik "no" ver ve normal test moduna devam et (kablo soruları da
    // atlanır). Bekleme, donanımın hazır olması için pay bırakır.
    printf("[AUTO] ATE secimi otomatik: %d saniye beklenip 'no' verilecek "
           "(normal test modu).\n", ATE_AUTO_WAIT_S);
    fflush(stdout);
    sleep(ATE_AUTO_WAIT_S);
    g_ate_mode = false;
    printf("[AUTO] ATE mode = NO. Normal test moduna devam ediliyor.\n\n");
    return;
#endif
