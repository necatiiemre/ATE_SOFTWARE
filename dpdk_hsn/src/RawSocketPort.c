...
#include "AteMode.h"  // for ate_mode_enabled()
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
void *raw_tx_worker(void *arg)
{
    ...
    // ATE modunda port basina tek target var (P12->P14 vs.); catch-up bütçesi
    // 4 ile sinirlandiginda BATCH_SIZE'a (16) ulasamiyor ve syscall basina
    // sadece 4 paket gondererek throughput'u yariya dusuruyor.
    // ATE: target sayisi az -> catch-up'i 16'ya cikar (toplam = BATCH_SIZE)
    // Normal: 4 target * 4 = 16 zaten BATCH_SIZE'a esit -> degisme
    const uint32_t MAX_CATCHUP_PER_TARGET = ate_mode_enabled() ? 16 : 4;
#endif
